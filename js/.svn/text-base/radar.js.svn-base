var RadarChart = {
	...
	draw: function(id, d, options) {
	    ...
	    var g = d3.select(id).append("svg").attr("width", cfg.w).attr("height", cfg.h).append("g").attr("class","topG");
		...
		for(var j=0; j<cfg.levels; j++) {
			...
			g.selectAll(".levels").data(allAxis).enter().append("svg:line")
				...
				.attr("class", "line").style("stroke", "black").style("stroke-width", "0.5px")
		...
		axis.append("text").attr("class", "legend")
			.text(function(d){return d}).style("font-family", "sans-serif").style("font-size", "12px").style("font-weight", "bold").attr("transform", function(d, i){return "translate(0, -10)"})
			.attr("x", function(d, i){return labelPosX[i];})
			.attr("y", function(d, i){return labelPosY[i];});
